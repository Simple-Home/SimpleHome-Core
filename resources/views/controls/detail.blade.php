            ...
            <div
                ...
                <div class="btn-group" role="group" aria-label="Standard Button Group">
                    <a style="font-weight:bold"
                        class="h3 btn {{ request()->segment(count(request()->segments())) == 'day' ? 'btn-primary' : 'btn-outline-primary' }} my-auto {{ $property->history >= 1 ? '' : 'disabled' }}"
                        id="period_today"
                        href="{{ route('controls.detail', [$property->id, 'day']) }}">{{ __('simplehome.period.day') }}</a>
                    <a style="font-weight:bold"
                        class="h3 btn {{ request()->segment(count(request()->segments())) == 'week' ? 'btn-primary' : 'btn-outline-primary' }} my-auto {{ $property->history >= 7 ? '' : 'disabled' }}"
                        href="{{ route('controls.detail', [$property->id, 'week']) }}">{{ __('simplehome.period.week') }}</a>
                    <a style="font-weight:bold"
                        class="h3 btn {{ request()->segment(count(request()->segments())) == 'month' ? 'btn-primary' : 'btn-outline-primary' }} my-auto {{ $property->history >= 29 ? '' : 'disabled' }}"
                        href="{{ route('controls.detail', [$property->id, 'month']) }}">{{ __('simplehome.period.month') }}</i></a>
                    <a style="font-weight:bold"
                        class="h3 btn {{ request()->segment(count(request()->segments())) == 'year' ? 'btn-primary' : 'btn-outline-primary' }} my-auto disabled {{ $property->history >= 365 ? '' : 'disabled' }}"
