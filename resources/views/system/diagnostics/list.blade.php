...
@section('content')
    <div id="server-dashboard" data-chart-endpoint="{{ route('system.diagnostics.chart.data') }}">
        ...
        <div class="row row-cols-1 row-cols-md-3">
            ...
            <div class="col mb-4">
                <div class="card">
                    ...
                    <div class="card-body">
                        {{ __('simplehome.publicIp') }}: <a
                            href="https://{{ $services['public_ip'] }}">{{ $services['public_ip'] }}</a><br>
                        {{ __('simplehome.networkIp') }}: <a
                            href="https://{{ $services['internal_ip'] }}">{{ $services['internal_ip'] }}</a><br>
                        {{ __('simplehome.hostname') }}: <a
                            href="https://{{ $services['hostname'] }}">{{ $services['hostname'] }}</a><br>
                        SSL: <span
                            class="{{ !$ssl ? 'text-danger' : 'text-success' }}">{{ $ssl ? __('simplehome.server.sslOn') : __('simplehome.server.sslOff') }}</span><br>
                    </div>
                </div>
            </div>
            <div class="col mb-4">
